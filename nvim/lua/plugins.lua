...
return require('lazy').setup({
...

  {
    ...
    config = function(_, opts)
      -- Monkeypatch for Neovim PR #39485 "refactor(globals): SearchState" (merged
      -- 2026-07-16), which folded the loose search globals (search_match_lines,
      -- search_match_endcol, ...) into a `SearchState` struct exported as `Search`.
      -- flash.nvim's flash/hacks.lua still FFI-reads the old standalone symbols → on
      -- such a build every flash motion errors:
      --   dlsym(RTLD_DEFAULT, search_match_lines): symbol not found
      -- This mirrors the upstream fix (flash.nvim PR #492): read the struct via
      -- `ffi.C.Search.*`, keeping flash's original end-position math (exact, and
      -- incsearch save/restore keep working). Self-healing: only patches when the old
      -- symbol is missing → no-op once flash ships #492 or on older Neovim.
      -- Remove this once flash.nvim merges PR #492.
      ...
      if vim.fn.exists("*search_match_endcol") == 0 then
