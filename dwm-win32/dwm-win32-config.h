...
static bool showclock                 = false;     /* false means no clock */
...
rotatelayout(const Arg *arg)
{
    ...
    for(int i = 0; i < LENGTH(layouts); i++) {
        if (&layouts[i] == lt[sellt]) {
            layout = &layouts[(i + 1) % LENGTH(layouts)];
            break;
        }
    }
...
static Key keys[] = {
    ...
    { MODKEY|MOD_SHIFT,             'R',       refreshdisplay,        {0 } },
    //{ MODKEY,                       'H',       setmfact,            {.f = -0.05} },
    //{ MODKEY,                       'L',       setmfact,            {.f = +0.05} },
    { MODKEY|MOD_SHIFT,                       'I',       showclientinfo,      {0} },
    ...
    */
    ...
    { MODKEY|MOD_CONTROL,           VK_SPACE,  rotatelayout,      {0} },
    /*
