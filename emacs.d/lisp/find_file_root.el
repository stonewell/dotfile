...
(defun find-alternative-file-root ()
  ...
  (let ((fname (or buffer-file-name
		   dired-directory)))
  (when fname
    (if (string-match (concat "^" find-file-root-prefix) fname)
	   (setq fname (replace-regexp-in-string
	     (concat "^" find-file-root-prefix) ""
	     fname)))
    (setq fname (concat find-file-root-prefix fname))
...
(defun find-file-hook-root-header-warning ()
  (when (and buffer-file-name (string-match (concat "^" find-file-root-prefix) buffer-file-name))
